...
3. **Cookie** - использует заголовки в http-запросах, для аутентификации и сохранении информации о сеансе.
...
4. **mTLS (Mutual Transport Layer Security)** - добавляет взаимную аутентификацию клиента и сервера. 
...
5. **Bearer token** - уникальный ключ для аутентификации и авторизации доступа к ресурсам
Клиент            -> сервер
basic auth        ->
                  <- Bearer token
запросы с токеном ->
...
6. **JWT токен** - подвид Bearer token, использует механизм подписи
Структура JWT:
header: 
  alg - алгоритм шифрования
  typ - тип токена
payload: инфа о пользователе, время жизни токена и кастомные поля
verify signature: подпись (ключ)
...
7. **openID Connect (OIDC)** - протокол аутентификации поверх OAuth 2.0. Позволяет приложению не только получать доступ к данным пользователя, но и узнавать, кто именно вошёл в систему
...
2. **ABAC** - на основе аттрибутов
+ более гибкая детализация доступов
- усложняется управление в большой системе 
3. **MAC** - обязательный контроль доступа для авторизации с высоким уровнем безопасности
4. **CBAC** - на основе контекста (например доступ только с 1 устройства)
5. **oAuth** - протокол авторизации, позволяющий приложению получать доступ к ресурсам в другом приложении (от имени пользователя).
