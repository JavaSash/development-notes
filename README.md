...
[вверх](#оглавление)
### Пагинация
Постраничный вывод данных клиенту.
Limit - количество элементов на стр. Стоит ограничивать верхнюю границу лимита для клиента, чтобы не терялся смысл пагинации.
Offset - сдвиг на количество элементов (номер стр), с которого отдать следующую пачку данных
Count - общее количество элементов. Возвращает сервер в ответе. По лимиту и каунту можно рассчитать общее количество страниц (limit*count)
Курсор - внутренний идентификатор последней записи на стр. (базовый сценарий). Клиент передаст курсор (элемент, с какого нужно выдать следующий пакет данных) вместо оффсета. Применим на больших объёмах данных или при частом обновлении данных в БД. В идеале само значение курсора шифровать от клиента.
...
Очереди сообщений (RabbitMQ, Apache Kafka) - это промежуточное ПО, которое обеспечивает асинхронную связь между распределенными системами. Они разделяют производителей и потребителей, позволяя им работать независимо друг от друга.

[вверх](#оглавление)
## Async HTTP API
**Callback**
Запрос к серверу (передаём callback_url) 
-> сервер отвечает клиенту сразу (например 202 Accepted) 
-> сервер обрабатывает запрос 
-> сервер отправляет ответ на callback_url из первого запроса клиента

Плюсы:
+ уменьшает задержки (сервер не держит клиента пока обрабатывает запрос)
+ клиент и сервер могут выполнять другие задачи в ожидании ответа
+ гибкость и адаптивность (подходит для обработки разных типов асинхронных задач)

Минусы:
- callback hell (множество вложенных обратных вызовов без должной обработки)
- проблема обработки ошибок (ошибки должны распространяться через несколько уровней обратных вызовов)
- производительность (чрезмерное использование может привести к узким местам в производительности)

Устранение недостатков через Promises, Async\Await механизмы 

**Polling**
Клиент устанавливает интервал опроса, опрашивает по GET API сервер
-> сервер получает GET запрос от клиента
-> если новые данные доступны, возвращает их клиенту
-> если новых данных нет, сообщает клиенту об этом
-> клиент обрабатывает ответ от сервера
-> если пришли новые данные обновляет веб-страницу (совершает нужное действие)
-> если нет новых данных клиент ничего не делает
-> клиент продолжает опрос сервера

Плюсы:
+ легко реализовать
+ широкая совместимость (стандартные HTTP-запросы)
+ нестационарная коммуникация (опрос поддерживает HTTP без статических данных)
+ дружественный брандмауэр (опрос используется тот же протокол (HTTP\HTTPS), что и веб трафик без доп настроек)

Минусы:
- неэффективное использование ресурсов (опрос генерирует большое количество ненужных запросов)
- задержка (клиент раньше следующего опроса не получит новые данные)
- проблема масштабируемости (больше клиентов = больше запросов на 1 опрос)
- неточное время (время между обновлениями может меняться из-за разных факторов - задержки сети и т.п.)
- непредсказуемая нагрузка на сервер (сложно предсказать при изменении количества клиентов и частоты опросов)

**Long Polling**
Клиент устанавливает таймаут (время ожидания изменений) и отправляет запрос на GET API сервера
-> сервер получает GET запрос от клиента
-> начинает проверку наличия новых данных в течение установленного клиентом таймаута (если данные появятся раньше, то сразу отправляет ответ)
-> если новые данные появились в течение этого времени, возвращает их клиенту
-> если новых данных не было - сообщает клиенту об этом
-> клиент обрабатывает ответ от сервера
-> если пришли новые данные обновляет веб-страницу (совершает нужное действие)
-> если нет новых данных клиент ничего не делает
-> клиент снова отправляет запрос на ендпоинт длинного опроса сервера

Плюсы:
+ обновление в реальном времени (если новые данные появились до конца времени ожидания - сервер сразу вернёт их клиенту)
+ снижение нагрузки на сервер (меньше количество ненужных запросов к серверу)
+ проще в реализации (например чем веб-сокеты)

Минусы:
- не совсем реальное время (передача данных всё ещё не в режиме реального времени по сравнению с WebSockets или Server-Sent Events)
- повышенная нагрузка на сервер (накладные расходы на поддержание открытых соединений)
- неэффективность при быстро меняющихся данных
- ограниченная масштабируемость (проблемы при поддержании открытых соединений для большого количества клиентов)
- обработка таймаута (серверу и клиенту нужно обрабатывать тайматы запросов и перезапускать коммуникацию)
...
Преимущества:
+ Простота
+ Гибкость (независимость от языка и платформы)
+ Производительность
+ Надёжность
+ Масштабируемость (из-за stateless, сервер не хранит состояние клиента)

Недостатки:
- Ограниченная функциональность (не поддерживает обмен данными в реальном времени)
- Нет строгости (REST это набор рекомендаций, а не стандартов)
...
2. READ
  ...
  * HEAD - частичное чтение, возвращает ответ без тела (только заголовки и код статуса).
    Также можно запрашивать коллекции, например HEAD /users. Идемпотентный, безопасный, кешируемый
  * OPTIONS - показывает доступные методы конкретного ресурса (указан апи путь) или всего сервера при *. Вернет 200 с заголовками, Allow: Get, post и тп. Безопасный, идемпотентный, не кешируемый.
...
5. LIST
  * GET + параметры. При необходимости передавать фильтры\параметры с чувствительной информацией, лучше делать это в боди и через POST, т.к. параметры запроса видны в истории браузера и всем промежуточным серверам.
...
HTTPS=HTTP+TLS
TLS при этом надстройка над http

В HTTPS используется гибридное шифрование:
1. Асимметричное шифрование для обмена ключами: при использовании сайта с HTTPS, браузер и сервер начинают процесс рукопожатия TLS (Transport Layer Security). В рамках этого процесса сначала используется асимметричное шифрование для безопасного обмена симметричными ключами. Ваш браузер получает открытый ключ сервера, содержащийся в сертификате безопасности, проверяет его подлинность, а затем использует этот ключ для шифрования и отправки серверу сгенерированного симметричного ключа. 
2. Симметричное шифрование для передачи данных: после передачи симметричного ключа и его получения обеими сторонами, вся последующая передача данных между браузером и сервером происходит с использованием симметричного шифрования. Это обеспечивает высокую скорость и эффективность, поскольку симметричное шифрование требует меньше вычислительных ресурсов по сравнению с асимметричным.

**Проблема отрицания** - когда отправитель говорит, что не отправлял информацию, а получатель её получил. Кто-то другой под видом отправителя отправил?
Эта проблема решается электронной подписью. 
Логика обмена сообщениями с электронной подписью:
1. Отправитель берёт хеш своего открытого сообщения и шифрует закрытым ключом отправителя (это его эл. подпись)
2. Вместе с сообщением отправитель шлёт и свою эл. подпись
3. Получатель с помощью открытого ключа отправителя расшифровывает сообщение и вычисляет хеш сообщения
4. Получатель из зашифрованного сообщения (не из эл. подписи) с помощью открытого ключа получателя расшифровывает сообщение и берёт его хеш
5. Получатель сравнивает хеши из п.3 и п.4

Для эл. подписи используются отдельные ключи, алгоритм получения и работы с ними:
1. Отправитель обращается в центр регистрации (компания с лицензией на выдачу ЭП) за получением ЭП (эл. подписи)
2. Центр регистрации передаёт данные в центр сертификации
3. Центр сертификации выдаёт сертификат (с открытым ключом и данными отправителя) и закрытый ключ (обычно на флешке)
4. Отправитель отправляет документ с ЭП
5. Отдельным запросом отправляет сертификат из п.3 с открытым ключом
6. Получатель решает на своей стороне проблему отрицания (см. выше)
7. Дополнительно можно проверить сертификат отправителя через реестр выданных и недействительных сертификатов (на случай, если сертификат украден\просрочен)
...
[вверх](#оглавление)
### Шифрование
**Кодирование** - процесс преобразования данных из одной формы в другую, обычно с целью передачи или хранения данных в определённом формате. Кодирование обратимо, и основная цель — сохранение формата данных, а не их секретность.
**Хеширование** - процесс преобразования входных данных любого размера в строку фиксированной длины (хеш). Этот процесс необратим, то есть вы не можете восстановить исходные данные из хеша.
**Шифрование** - процесс преобразования информации в зашифрованный вид с помощью алгоритма шифрования и ключа, который делает содержимое недоступным для неавторизованных лиц. Шифрование обратимо, то есть информацию можно расшифровать обратно в исходное состояние, но только с использованием соответствующего ключа.

**Симметричное шифрование** - отправитель и получатель используют 1 ключ для шифрования и дешифрования данных
Плюсы:
+ быстрое
...
На основе публичного и приватного ключей. У отправителя и получателя свой набор этих ключей.
Логика работы:
1. Получатель отправляет свой публичной ключ отправителю
2. Отправитель с помощью публичного ключа получателя шифрует сообщение и отправляет получателю
3. Получатель на основе своего закрытого ключе расшифровывает сообщение
4. В обратную сторону аналогично

Плюсы:
+ высокий уровень безопасности
+ не боимся потерять открытый ключ (им нельзя расшифровать сообщение, только зашифровать)
+ предпосылки для подтверждения личности (электронная цифровая подпись)

Минусы:
- медленное
- требует больших мощностей
- сложнее в управлении ключами
- непригодно для больших данных

[вверх](#оглавление)
### Security
Снижение рисков утечки данных:
1. Запрет всех неиспользуемых HTTP-методов
2. Использование HTTPS (заголовок Strict-Transport-Security: max-age=63072000(год);IncludeSubDomains;preload)
3. Не раскрывать параметры в URL (передавать чувствительную инфу в заголовках)
4. Спец. ссылки для восстановления доступа
   * Генерировать уникальные ссылки (с юидами, а не с порядковым номером)
   * https
   * исключены из индексации поисковиков
   * гибкое управление (функционал деактивации ссылки)
5. Cookie
  * уникальный идентификатор сессии
  * новый вход - новая сессия
  * HttpOnly (запрещает получать куки через скрипт)
  * SameSite (запет передачи другим сайтам)
  * заголовок csrf_token=уникальное_значение с проверкой на клиенте (javaScript). Защита от кражи куки
6. Защита сайта от исполняемых скриптов. Content Security Policy (заголовок) - указываем безопасные источники загрузки данных
7. CORS (Cross-Origin Resource Sharing) - защищает ресурсы на сервере от несанкционированного доступа с других доменов.
   Работает в случаях:
  * отличается протокол (http\https)
  * отличается порт
  * отличается хост
8. Передача конфиденциальных данных в URL отображается в заголовке referrer. Выставляется Referrer-Policy: no-referrer
9. Заголовок Permissions-Policy: - указывает какие функциональности браузера можно использовать на сайте
10. Заголовок X-Content-Type-Options: nosniff. Content-Type обязательно должен соблюдаться и не должен изменяться
11. Заголовок X-Frame-Options: sameorigin. Нужно отображать страницу только, если родительские элементы страницы имеют одинаковое происхождение с самой страницей. Т.е. запрещает встраивать страницу в другие фреймы
12. Журналирование.


[вверх](#оглавление)
### Авторизация и аутентификация
**Идентификация** - по уникальному идентификатору понять, что вы это вы. Например, по логину

**Аутентификация** - механизм проверки личности. Например, по связке логин+пароль

**Авторизация** - выдача лицу прав на выполнение определённых действий

Обычно идёт в таком порядке: идентификация, аутентификация, авторизация. Но есть сервисы, где нужна только авторизация (например, только просмотр файлов по ссылке в общедоступном файловом хранилище)

**Виды аутентификации**
1. ## Basic auth
Заголовок Authorization с префиксом Basic и строки логин:пароль, закодированной в Base64. Данные не безопасны, не зашифрованы и их легко декодировать
- слабая безопасность (не для конфиденциальных\чувствительных данных)
- быстрая и простая реализация
- сложность управления сессией (при каждом запросе нужно отправлять заголовок, браузер может его кешировать, нет TimeToLive)
2. ## API-key
Сервис выдаёт уникальный ключ -> клиент использует этот ключ в запросах к API
Сервис сам решает когда отозвать действие этого ключа.
Часто используется в разработке, для тестирования интеграций.
- простота и подходит для разработчиков
- сложность авторизации (сложно разделять права)
- риски безопасности (утечка API-key)
3. ## Cookie
использует заголовки в http-запросах, для аутентификации и сохранении информации о сеансе.
Сервер в ответ на клиентский запрос отправляет заголовок "Set-Cookie" с SessionId и доп информацией. Далее клиент должен в каждом последующем запросе передавать куки с id сессии.
- можно гибко управлять сессией (выход + ограничение сессии по времени)
- гибкость авторизации (можно в куки шифровать данные по правам пользователя и т.п.)
- нужно хранить сессию клиента (накладные расходы, становится stateful)
4. ## mTLS 
(Mutual Transport Layer Security) - добавляет взаимную аутентификацию клиента и сервера. 
В отличие от обычного TLS, где только сервер предоставляет сертификат для аутентификации, в mTLS и клиент, и сервер должны предоставить свои сертификаты.

Используется для межсервисного взаимодействия, а не клиент-серверного.
Сервер 1  -> сервер 2
          -> запрос защищённого соединения
          <- запрос серта у с1 и показ своего
          -> проверка серта с2 и показ своего серта
          <-проверка серта с1, установка защищённого соединения

+ серьёзный уровень безопасности
+ stateless
- сложная и дорогая реализация
- сложность управления сертификатами, сроками жизни

Сертификат mTLS выглядит, как обычный сертификат TLS. Содержит инфу:
* Субъект — кому выдан сертификат (человек, организация или устройство). 
...
5. ## Bearer token
уникальный ключ для аутентификации и авторизации доступа к ресурсам
Клиент            -> сервер
basic auth        ->
                  <- Bearer token
запросы с токеном ->

Возможно усилить безопасность и выдавать 2 токена (текущий для доступа и второй для обновления первого).
+ гибкое управление сессией (принудительным выходом при компрометации и временем жизни)
+ гибкая авторизация (можно зашифровать роли и доп инфу)
- нужно хранить состояние на сервере
6. ## JWT
Подвид Bearer token, использует механизм подписи. Предназначение JWT - проверка подлинности источника данных. 
При этом стоит понимать, что токен кодируется и подписывается, но не шифруется. Поэтому передавать в нём конфиденциальную информацию не стоит.
...
```
Полезная нагрузка (payload) - хранится информация о пользователе, которую сервер аутентификации передает серверу приложения.
...
* sub – уникальный идентификатор пользователя.
  ...
  Подпись - вычисляется на основе его заголовка и полезной нагрузки по следующей схеме
```
...
```
Cборка JWT. На основании данных из п1-3 собираем веб-токен по схеме
```
header.payload.signature
```
Заголовок и полезная нагрузка предварительно кодируются в Base64URL, а подпись уже в правильном формате.
Верификация JWT. В примере выше использовалось хеширование с секретным ключом по алгоритму HS256. 
Изначально этот ключ известен только серверу аутентификации. Сервер приложения получает его при настройке процесса проверки подлинности.

Когда пользователь отправляет запрос с прикрепленным к нему JWT, приложение может самостоятельно произвести хеширование данных и сравнить результат с полученной подписью. 
Если строки совпадают, значит вызов поступил из подтвержденного источника и может быть выполнен безопасно.

![Схема авторизации через JWT](/jwt.png)
[Первоистичник](https://proglib.io/p/jwt-for-dummies)

+ stateless (не хранит данные о пользователе, а с помощью ключа берёт их из токена)
+ междоменная совместимость
+ гибкая авторизация
+ усиленная безопасность
- сложная реализация
- нагрузка на сеть
- ограниченное время жизни (нет гибкого управления сессией)
- нельзя ставить бесконечное время жизни - не отозвать, т.к. stateless
7. ## OIDC
openID Connect - протокол аутентификации поверх OAuth 2.0. Позволяет приложению не только получать доступ к данным пользователя, но и узнавать, кто именно вошёл в систему
...
+ быстро внедрить
+ легко реализовать
- сложно реализовать при нескольких продуктах (в разных продуктах у клиента могут быть разные роли)
2. ## ABAC
Attribute-based access control - на основе аттрибутов
+ более гибкая детализация доступов
- усложняется управление в большой системе 
3. ## MAC
обязательный контроль доступа для авторизации с высоким уровнем безопасности
4. ## CBAC
на основе контекста (например доступ только с 1 устройства)
5. ## oAuth
протокол авторизации, позволяющий приложению получать доступ к ресурсам в другом приложении (от имени пользователя).
Причём пользователь авторизуется только 1 раз в приложении, а дальше общение между приложениями идёт через токен.
